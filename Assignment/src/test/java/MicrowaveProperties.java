...
public class MicrowaveProperties {
	...
	public void checkProperties(InputRecord previousInput, OutputRecord previousOutput,
		...
		// Property: If time to cook is zero, the mode must be Setup
		...
		// Property: When cooking, time either stays the same or decreases by exactly one
		...
		// Property: Time to cook must always be non-negative
		assertTrue("seconds to cook is always >= 0", 
					currentOutput.timeToCook >= 0);

		// Property: Clear button in suspended mode transitions to setup mode
		...
		// Property: Start button in suspended mode with closed door and time > 0 enters cooking mode
		...
		// Property: Start button in suspended mode with open door remains in suspended mode
		assertTrue("If in suspended mode, at the instant the start key is pressed " + 
		...
		// Property: Clear button in setup or suspended mode resets time to zero
		...
		// Property: In setup mode with no button presses, state remains unchanged
		...
		// Property: When cooking continuously, time will eventually decrease within the tick rate
